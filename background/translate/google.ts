...
export async function googleTranslate(text: string, from: string, to: string): Promise<string> {
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`Google翻译请求失败: ${res.status} ${errText}`);
    }
    ...
    console.error("Google翻译返回内容异常: " + JSON.stringify(data));
  } catch (error) {
...
export async function googleTranslateBatch(texts: string[], from: string, to: string): Promise<string[]> {
  try {
    ...
    if (!res.ok) {
      const errText = await res.text();
      console.error(`Google批量翻译请求失败: ${res.status} ${errText}`);
    }
    const data = await res.json();
    // 返回批量翻译结果数组
    ...
    console.error("Google批量翻译返回内容异常: " + JSON.stringify(data));
  } catch (error) {
