...
const requestHandle = async <TData>(body: HandleRequestBody): Promise<HandleResponse<TData>> => {
...
export async function callTranslateAPI(
  ...
  try {
    ...
    throw (isExtensionContextInvalidatedError(error) ? toContextInvalidatedError() : error);
  }
}
...
export async function callTTSAPI(
  text: string,
  lang: string,
  speechOptions: SpeechRequestOptions = {}
